export const APP_CONSTANTS = {
  ...
  UI: {
    ...
    HELP: {
      ...
      LINES: [
        'Сначала создайте занятие кнопкой "Добавить учебное занятие", без него отметки ставить некуда.',
        "Один клик по ячейке - быстро поставить отметку. Клик по красной ячейке открывает окно с отметками и посещаемостью.",
        "Двойной клик по ячейке - открыть окно с: посещаемостью, основной отметкой и пересдачами.",
        "Красная ячейка - обучающийся отсутствует, причина написана прямо в ячейке.",
        "Удалить занятие можно из окна его редактирования. Удаление необратимо.",
      ],
      ...
      ATTENDANCE_LINES: [
        "Клик по любой ячейке таблицы - отметить пропуск (повторный клик по отмеченной - снять его).",
        "Область применения: только эта пара, диапазон пар этого дня или диапазон дней - отметка проставится сразу на все выбранные занятия.",
        'Перед сохранением смотрите "Будет затронуто записей" - столько записей изменится.',
        "Период таблицы ограничен 31 днем.",
      ],
    },
    DELETE_LESSON: {
